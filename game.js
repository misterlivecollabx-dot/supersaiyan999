...
function resizeCanvas() {
  ...
  const baseScale = Math.max(0.72, Math.min(1.15, shellWidth / 390));
  const characterHeight = Math.max(130, Math.min(Math.floor(availableArenaHeight * 0.70), Math.floor(360 * baseScale)));
  const characterWidth = Math.max(110, Math.min(400, Math.floor(characterHeight * 0.65)));
